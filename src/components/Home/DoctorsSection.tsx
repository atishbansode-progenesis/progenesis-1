...
const DoctorsSection = () => {
  ...
  return (
    <section className="py-20 bg-[#F9FAFB] px-6 lg:px-[50px] xl:px-[80px] 2xl:px-[120px]">
      {/* Heading */}
      <div
        className="flex flex-col md:flex-row items-start justify-between gap-8 mb-[144px]"
        ref={ref}
      >
        {/* Left */}
        ...
        {/* ✅ Static version with Tailwind responsive typography */}
        <div className="w-full md:w-[55%] flex items-center">
         <p
  className="
    text-[#2C2C2C] font-[Manrope]
    text-base sm:text-lg md:text-[32px]
...
>
  {text}
      ...
      <div className="relative flex items-center">
        <button
    onClick={prevDoctor}
    className="absolute left-[calc(50%-760px)] w-[48px] h-12 flex items-center justify-center rounded-lg 
               border border-blue-600 text-blue-600 hover:bg-blue-50 transition-all duration-300 z-10"
  >
    ←
  </button>

        {/* <div
          className="bg-white rounded-2xl flex flex-col md:flex-row overflow-hidden shadow-sm border border-gray-200 w-full lg:max-w-[1000px] 2xl:max-w-[1400px] mx-auto px-4 sm:px-6 md:px-10 2xl:px-20 py-6 md:py-10 relative transition-all duration-300"
        > */}
        <div
          className="bg-white rounded-2xl flex flex-col md:flex-row overflow-hidden shadow-sm border border-gray-200 
             w-full max-w-[1398px] h-[476px] flex-shrink-0 mx-auto relative transition-all duration-300"
        >

          <p className="absolute -top-5 right-5 text-[#606060] font-[Manrope] text-sm sm:text-base font-semibold tracking-[-0.32px]">
            {String(currentIndex + 1).padStart(2, "0")} of{" "}
            {String(doctors.length).padStart(2, "0")}
          </p>

          <div className="flex-shrink-0 p-[6px] bg-white h-[476px] flex items-center justify-center">
            <div className="bg-[#27C3EB] rounded-xl overflow-hidden w-[calc(100%-12px)] h-[full]">
              <img
                src={doctor.image}
                alt={doctor.name}
                className="h-[444px] w-auto object-cover rounded-xl"
                loading="lazy"
              />
            </div>
          </div>
          ...
          {/* <div className="flex flex-col justify-between flex-1 px-3 sm:px-6 md:px-10 mt-4 md:mt-0 w-[50%]">
            <h3 className="text-[#1656A5] font-[Manrope] text-2xl sm:text-3xl md:text-4xl 2xl:text-5xl leading-tight tracking-tight">
              {doctor.name}
            </h3>

            <p className=" text-[#606060] font-[Manrope] text-sm sm:text-base leading-relaxed tracking-tight">
              {doctor.description}
            </p>
  ...
  {doctor.experience.map((exp, idx) => (
    <div
      key={idx}
      className="w-fit px-4 py-2 rounded-xl bg-[rgba(22,86,165,0.05)] 
                 text-[#1656A5] font-[Manrope] text-sm backdrop-blur-[25px]"
    >
      {exp}
    </div>
  ))}
          ...
          <div
            className="flex flex-col justify-start flex-1 px-[24px] pt-[24px] w-[50%] h-full"
          >
            {/* Doctor Name */}
            <h3
            ...
            {/* Description */}
            <p
              className="mt-3 text-[#606060] font-[Manrope]
            ...
            <div
              className="absolute bottom-[12px] flex flex-col gap-3 translate-y-[-6px]"
            >
              {doctor.experience.map((exp, idx) => (
                <div
                  key={idx}
          ...
          {/* Divider Line */}
          <div className="hidden md:block w-[1px] h-[400px] bg-black/25 self-center"></div>

          {/* Location Section */}
          <div className="relative flex items-end w-[30%] h-full">
            <div
              className="absolute left-[24px] bottom-[30px] transition-all duration-500 ease-in-out"
            >
              <h4
        ...
        <button
    onClick={nextDoctor}
    className="absolute right-[calc(50%-760px)] w-[48px] h-12 flex items-center justify-center rounded-lg 
               border border-blue-600 text-blue-600 hover:bg-blue-50 transition-all duration-300 z-10"
  >
    →
  </button>
