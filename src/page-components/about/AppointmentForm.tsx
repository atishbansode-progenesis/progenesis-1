...
//     short.toUpperCase().split('').map(c => String.fromCodePoint(0x1F1E6 + c.charCodeAt(0) - 65)).join('');
...
//             <>
//               <select
//                 className="absolute left-3 top-1/2 -translate-y-1/2 z-10 bg-transparent border-0 focus:outline-none text-sm appearance-none w-16 h-8"
//                 onChange={(e) => handleChange(`${field.name}_code`, e.target.value)}
...
//           <input
//             type={field.type}
//             placeholder={field.type === "tel" ? "Enter phone number" : (field.placeholder || field.title)}
//             required={field.required}
//             className={`${containerClass} ${field.type === "tel" ? "pl-16" : ""}`}
            ...
            {filteredCountries.length > 0 ? (
              filteredCountries.map((country) => (
                <li key={country.short}>
                  ...
                  >
                    <span className="text-sm">{getFlagEmoji(country.short)}</span>
                    <div className="flex-1">
