...
const AdDoctorSection = () => {
  ...
  const doctors = [
    ...
    },
    {
      ...
      description: "He is a highly experienced fertility expert specializing in reproductive surgery and complex fertility cases.",
      experience: [
        "14+ Years",
    ...
    },
    {
      ...
      image: "/images/doctors/drshital.jpg",
      description: "She provides ethical and compassionate fertility care with a focus on patient well-being.",
      experience: [
        "10+ Years",
    ...
    },
    {
      ...
      description: "She is dedicated to advanced fertility treatments and helping couples achieve parenthood goals.",
      experience: [
        "13+ Years",
        "MS (Obstetrics and Gynaecology)",
        "Fellowship in Reproductive Medicine"
    ...
    },
    {
      ...
      description: "She adopts scientifically proven methods for the treatment of infertility to ensure better outcomes.",
      experience: [
        "7+ Years",
        "DGO (Obstetrics and Gynaecology)",
        "Fellowship in Reproductive Medicine"
    ...
    },
    {
      ...
      description: "He uses cutting-edge reproductive technologies in combination with patient-centered care.",
      experience: [
        "10+ Years",
        "DGO (Obstetrics and Gynaecology), Bachelor of Medicine",
        "Fellowship in Reproductive Medicine"
      ],
      location: "Amravati",
      role: "Fertility Consultant"
    },
    {
      ...
      description: "She is dedicated to providing holistic care with modern reproductive techniques.",
      experience: [
        "6+ Years",
        "DGO (Obstetrics and Gynaecology)",
        "Fellowship in Reproductive Medicine"
    ...
    },
    {
      ...
      description: "He delivers tailored fertility treatments with a focus on improving patient outcomes.",
      experience: [
        "8+ Years",
        "DGO (Obstetrics and Gynaecology)",
        "Fellowship in Reproductive Medicine"
  ...
  // Filter doctors based on UTM parameter
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const utmCity = params.get('utm_city');

    if (utmCity) {
      // Find Dr. Narhari (id: 1)
      const drNarhari = doctors.find(doctor => doctor.id === 1);
      
      // Filter doctors by city (case-insensitive comparison)
      const cityDoctors = doctors.filter(
        doctor => doctor.location.toLowerCase() === utmCity.toLowerCase()
      );

      // Combine: Dr. Narhari first (if not already in city doctors), then city doctors
      let filtered = [];
      if (drNarhari) {
        const isDrNarhariInCity = cityDoctors.some(doc => doc.id === 1);
        if (!isDrNarhariInCity) {
          filtered = [drNarhari, ...cityDoctors];
        } else {
          filtered = cityDoctors;
        }
      } else {
        filtered = cityDoctors;
      }
      ...
      // Show all doctors if no UTM parameter
  ...
  const prevDoctor = () => {
    setCurrentIndex((prev) => (prev === 0 ? filteredDoctors.length - 1 : prev - 1));
  };

  const nextDoctor = () => {
    setCurrentIndex((prev) => (prev === filteredDoctors.length - 1 ? 0 : prev + 1));
  };

  const handleScroll = () => {
    if (scrollContainerRef.current) {
      const scrollLeft = scrollContainerRef.current.scrollLeft;
      const cardWidth = scrollContainerRef.current.scrollWidth / filteredDoctors.length;
      const index = Math.round(scrollLeft / cardWidth);
      setCurrentIndex(Math.min(index, filteredDoctors.length - 1));
    }
  };

  // Don't render if no doctors are filtered
      ...
      <div className="flex flex-col lg:flex-row items-start justify-between gap-6 lg:gap-12 mb-8 md:mb-20 lg:mb-32">
        ...
        <div className="w-full lg:w-1/2 lg:max-w-2xl">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-light text-gray-900 mb-4 md:mb-6 leading-tight">
            Meet Our <br /> Fertility Specialists
          </h2>
          <button 
        ...
        <div className="w-full lg:w-1/2 flex items-center">
          <p className="text-gray-700 text-sm sm:text-base md:text-lg lg:text-2xl xl:text-3xl leading-relaxed md:leading-relaxed lg:leading-10 tracking-tight font-normal">
            Our center features a team of highly experienced fertility specialists dedicated to providing personalized care and advanced treatments to help you achieve your dream of parenthood.
          </p>
        </div>
      </div>
        ...
        <div className="md:hidden overflow-hidden">
          ...
          <div className="p-4 text-center">
            <p className="text-gray-600 text-xs font-semibold">
              ...
              {String(filteredDoctors.length).padStart(2, "0")}
            </p>
          </div>
        </div>
...
export default AdDoctorSection;